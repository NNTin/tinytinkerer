      ...
      {error ? (
        ...
        <div className="mt-5 flex items-center gap-3 rounded-[1.5rem] border border-[var(--widget-border)] bg-white px-4 py-3">
          ...
          <span className="text-xs text-[var(--widget-muted)]">
            Preparing the whiteboard and chat assistant.
          </span>
        </div>
      )}
    </div>
...
export const CanvasBootScreen = ({ error }: { error?: string }) => (
  <LoadingPanel
    ...
    message="Starting the shared browser shell before the whiteboard mounts."
    {...(error ? { error } : {})}
  />
)
...
export const CanvasRouteLoading = () => (
  <LoadingPanel
    ...
    message="Fetching the requested route and canvas controls."
  />
)
...
// Shown while the (lazy) Excalidraw chunk loads. It fills the window beneath the
// floating chat, which can mount and accept prompts before the canvas is ready.
export const CanvasSurfaceLoading = () => (
  <div className="flex h-full w-full items-center justify-center text-sm text-[var(--widget-muted)]">
