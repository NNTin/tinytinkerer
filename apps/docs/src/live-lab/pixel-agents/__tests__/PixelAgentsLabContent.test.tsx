...
vi.mock('../capability', () => ({ usePixelAgentsCapability: capability.usePixelAgentsCapability }))

vi.mock('../upstream-url', () => ({
  useResolveUpstreamUrl: () => (path: string) => `https://example.test/upstream/${path}`
}))

vi.mock('../loading-screen', () => ({ PixelAgentsLabChatLoading: () => null }))

import { PixelAgentsLabContent } from '../PixelAgentsLabContent'

describe('PixelAgentsLabContent', () => {
  afterEach(() => {
    vi.clearAllMocks()
  ...
  it('falls back to a text-only surface (no PixelAgentsStage) when capability says no', () => {
    ...
    // The assistant chat surface still renders on its own in fallback mode.
