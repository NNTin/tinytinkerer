...
describe('checkInboundRateLimit', () => {
  ...
  it('allows requests up to the limit and rejects the next with the remaining window', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      expect(await checkInboundRateLimit('auth', 'caller-a', 3, WINDOW_MS, nowMs)).toEqual({
        limited: false
      })
    }

    const limited = await checkInboundRateLimit('auth', 'caller-a', 3, WINDOW_MS, nowMs + 10_000)
  ...
  it('does not extend the window for rejected requests', async () => {
    ...
    const limited = await checkInboundRateLimit('auth', 'caller-a', 1, WINDOW_MS, nowMs + 2_000)
  ...
  it('opens a fresh window once the previous one elapses', async () => {
    const nowMs = 1_000_000
    await checkInboundRateLimit('auth', 'caller-a', 1, WINDOW_MS, nowMs)
    expect((await checkInboundRateLimit('auth', 'caller-a', 1, WINDOW_MS, nowMs + 1)).limited).toBe(
      true
    )
    expect(
      await checkInboundRateLimit('auth', 'caller-a', 1, WINDOW_MS, nowMs + WINDOW_MS + 1)
  ...
  it('keeps one caller’s exhaustion from limiting another (per-key isolation)', async () => {
    ...
    expect((await checkInboundRateLimit('search', 'caller-a', 1, WINDOW_MS, nowMs)).limited).toBe(
      true
    )
    expect(await checkInboundRateLimit('search', 'caller-b', 1, WINDOW_MS, nowMs)).toEqual({
      limited: false
    })
  })
  ...
  it('keeps scopes independent for the same caller', async () => {
    ...
    expect(await checkInboundRateLimit('mcp', 'caller-a', 1, WINDOW_MS, nowMs)).toEqual({
      limited: false
    })
  })
