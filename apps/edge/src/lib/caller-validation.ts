...
export type CallerValidationResult = 'valid' | 'invalid' | 'unavailable'
 ...
 * server-side resource on the caller's behalf — the model proxy (shared LiteLLM
 * key), the search proxy (shared Tavily key), and the MCP proxy (outbound
 * fetch) — so a merely well-formed Authorization header is never enough to use
 * those resources.
 *
