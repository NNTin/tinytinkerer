...
export default defineConfig(({ command }) => ({
  ...
  plugins: [
    ...
    // build: it pure-annotates `defineDiagnostics()` and guards report-only
    // diagnostic call sites with `process.env.NODE_ENV !== 'production'`, so the
    // production `build:pages` bundle tree-shakes out report-only diagnostics and
    // their dev reporters. Sentry + the edge error contract remain the prod
    // source of truth — see packages/app/app-browser/src/diagnostics.ts.
    nosticsStrip.vite(),
    // Dev-server only: collects diagnostics forwarded from the browser by the
    // dev reporter and appends them to `.nostics.log` for an agent to read.
