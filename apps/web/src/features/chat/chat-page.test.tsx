...
vi.mock('@tinytinkerer/app-browser', async () => {
  ...
  return {
      ...
      open ? (
        <div role="dialog" aria-label="Settings">
          ...
          <section role="region" aria-label="Auth">
            ...
            {mockAuthState.token ? null : <button type="button">Sign in with GitHub</button>}
          </section>
    ...
    // Stand-in for the shared turn chrome: renders the assistant content text and
    // threads turn.id through, mirroring the real component's contract.
    TurnChrome: ({
      turn
    }: {
      turn: {
...
describe('ChatPage settings modal', () => {
  ...
  it('shows auth controls in the real modal', async () => {
    ...
    expect(
      await within(authRegion).findByRole('button', { name: /sign in with github/i })
    ).not.toBeNull()
  })
...
describe('ChatPage composer submit (issue #206)', () => {
  ...
  it('replaces send with a Stop button while the agent is running and aborts on click', () => {
