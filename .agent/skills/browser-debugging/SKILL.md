...
2. Run the startup tool: `node .agent/skills/browser-debugging/tools/browser-login.mjs`. It opens the app, consents to the privacy notice if it pops up, and signs in with the PAT from `$TINYTINKERER_GITHUB_TOKEN` (or `.env.github` as a fallback).
...
- `tools/start-dev.sh` — preflight + start the dev servers. Frees stale ports (3111/8787), ensures deps + native builds (`sharp`/`esbuild`/`workerd` — pnpm skips those build scripts by default, which otherwise kills `generate:brand-assets` with `Cannot find package 'sharp'`), generates brand assets, starts `pnpm dev` in the background, and waits until both servers answer. Idempotent. Logs to `/tmp/tinytinkerer-dev.log`.
- `tools/browser-login.mjs [--port 3111] [--url <url>] [--token-file <path>]` — opens the web shell (`http://localhost:3111/web/`), accepts the telemetry/privacy consent if shown, and signs in via personal access token. Reads the PAT **by reference** from the `TINYTINKERER_GITHUB_TOKEN` (legacy `GITHUB_MODELS_TOKEN` still honoured) environment variable, falling back to `.env.github` (repo root) when it is unset. Self-heals a hung browser session (closes all + retries the open once). Verifies the session actually stuck and fails loudly with a scope diagnostic if the app drops the token.
...
- **Token by reference only.** The PAT comes from the `TINYTINKERER_GITHUB_TOKEN` (legacy `GITHUB_MODELS_TOKEN` still honoured) environment variable (exported in `~/.bashrc`), falling back to the gitignored `.env.github` file when unset. Never echo it, write it into the repo, or commit it. The value is only ever handed to the local browser. Note: tool-invoked bash is **non-interactive** so it does not source `~/.bashrc`, and `.env.github` is per-worktree — if login can't find the token, `source ~/.bashrc` or export it inline first (see `workflows/debug-pnpm-dev.md`).
