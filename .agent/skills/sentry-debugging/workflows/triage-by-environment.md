...
_before_ you spend effort on root cause. The two Sentry projects do **not** share
...
- **Frontend = 4 environments** (`production`, `develop`, `pr-preview`,
  ...
  by _which worker served them_, and there is no separate preview worker.
...
- **The `vercel-production` / `vercel-preview` labels are release _deploy_ labels**
