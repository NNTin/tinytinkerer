...
**Provider tags (model routes only).** `request_origin` is the provider the edge _resolved to_ (defaults to `github`); `request_provider` is what the client actually _asked for_. They diverge when the client omits the provider field: the edge stamps `request_provider: absent` + `provider_missing: true` on captured failures and **also emits a standalone `warning` Sentry message** (`<area> request omitted the provider field; defaulting to github`) so a misbehaving client is surfaced, not silently served as github. If you see `provider_missing: true`, the bug is the _caller_ not sending `provider` — fix the call site; don't chase the github adapter. The edge message sink (`apps/edge/src/lib/sentry.ts`) was added for this; the browser already had one.
