...
**`parse_error`/`schema_error` from *model output* (not a transport body) is its own case — and a trap: do NOT `accept` it.** When the `request_area` parses an LLM's free-form text into structured JSON (e.g. `react.decide`) and `http_status: 200`, the request succeeded — what failed is the model not complying (prose, empty, truncated stream, or wrong shape). A thrown parse error propagates through `nextDecision` (which only retries rate limits) and **kills the whole run**, so you must **recover to a graceful default** (`{ kind: 'final' }`, mirroring the runtime's `decision ?? { kind: 'final' }`) so the loop synthesizes an answer. First **parse robustly** (`robust-json.ts` `parseRobustModelJson`: strict → first-balanced-object + JSON5) to recover *sloppy-but-complete* output (prose-wrapped, single quotes, trailing commas) rather than needlessly dropping the action — but **never repair a truncated value** (no auto-closing brackets); a cut-off action must fall back, not run with a fabricated argument. But recovering does **not** make it acceptable noise: a *truncated* decision abandons the tool action the model was mid-way through emitting and answers from **incomplete tool results** — a real defect — and you can't tell that lossy case from a clean "finished in prose" at parse time. So **keep capturing both `parse_error` and `schema_error` (no `accept`)**: recover for the user, stay loud for us. Beware the two parses at these sites: the OpenAI-shaped **envelope** parse (`parseJsonWithTelemetry` of `{choices:[…]}`) must stay valid-JSON-or-bug too. Harden the streaming + non-streaming decider siblings together. Full SOP: `workflows/llm-decision-parse-error.md` (`FRONTEND-J`/`FRONTEND-K`).
