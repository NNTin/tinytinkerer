...
**`parse_error`/`schema_error` from *model output* (not a transport body) is its own case.** When the `request_area` parses an LLM's free-form text into structured JSON (e.g. `react.decide`) and `http_status: 200`, the request succeeded — what failed is the model not complying (prose, empty, truncated stream, or wrong shape). This is inherent & unavoidable, but a plain `accept` is **not** enough: a thrown parse error propagates through `nextDecision` (which only retries rate limits) and **kills the whole run**. Fix = **recover to a graceful default** (`{ kind: 'final' }`, mirroring the runtime's `decision ?? { kind: 'final' }`) so the loop synthesizes an answer, **then `accept: { kinds: ['parse_error'] }`** on the *decision-content* metadata. Keep `schema_error` **captured** (valid-JSON-wrong-shape can mean *our* contract drifted) but still recover from it. Beware the two parses at these sites: the OpenAI-shaped **envelope** parse (`parseJsonWithTelemetry` of `{choices:[…]}`) must stay valid-JSON-or-bug — never accept it; only the model-content parse is unavoidable. Harden the streaming + non-streaming decider siblings together. Full SOP: `workflows/llm-decision-parse-error.md` (settled `FRONTEND-J`/`FRONTEND-K`).
