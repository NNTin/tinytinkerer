...
//
// `-r` is required: a non-recursive `pnpm rebuild` only rebuilds packages in the
// root project's dependency closure, so workspace-only deps like workerd and
// @sentry/cli (pulled in by apps/edge, not the root) are silently skipped and
// their binaries never get set up.
run('pnpm', ['-r', 'rebuild', 'sharp', 'esbuild', 'workerd', '@sentry/cli'])
