...
export const getIdeFileRevision = (
...
export const applyWorkspaceChanges = (
  ...
  for (const change of changes) {
    ...
    const actualRevision = getIdeFileRevision(state.revisions, change.path)
  ...
  for (const path of touchedPaths) {
    if (path in nextFiles) nextRevisions[path] = getIdeFileRevision(state.revisions, path) + 1
