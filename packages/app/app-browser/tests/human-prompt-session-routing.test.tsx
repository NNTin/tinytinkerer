 ...
 *
 * `AppBrowserProvider` rather than `BrowserAppShell`: the shell's contribution
 * (whether it mounts the modal at all) is #479's `globalHosts` gate, covered by
 * `document-globals-multi-shell.test.tsx` and by docs' `no-human-prompt.test.ts`.
 * What #489 changed is which queue a mounted renderer reads, and the provider is
 * exactly the boundary that answers it.
 */
...
describe('human prompts route to the originating session (issue #489)', () => {
  it('renders a prompt only in the session that raised it, even on an identical conversation id', async () => {
    ...
    void assistant.stores.humanPrompts.getState().request(pollView('Assistant asks'), 'shared')
  ...
  it('takes the presentation choice and conversation label from the originating session', async () => {
    ...
    void assistant.stores.humanPrompts.getState().request(pollView('Assistant asks'), 'conv-a')
  ...
  it('settles only the originating session when a conversation with a shared id is reset', async () => {
    ...
    const assistantPrompt = assistant.stores.humanPrompts
      .getState()
      .request(pollView('Assistant asks'), 'shared')
    const labPrompt = lab.stores.humanPrompts.getState().request(pollView('Lab asks'), 'shared')
    ...
    expect(lab.stores.humanPrompts.getState().queue).toHaveLength(1)
  })
  ...
  it('lets two sessions hold a modal at once and settles them independently', async () => {
    ...
    const assistantPrompt = assistant.stores.humanPrompts
      .getState()
      .request(pollView('Assistant asks'), 'conv-a')
    const labPrompt = lab.stores.humanPrompts.getState().request(pollView('Lab asks'), 'conv-b')
  ...
  it('shows one session’s prompt in every surface of that session, and settles it everywhere at once', async () => {
    // Several SHELLS and several surfaces can share one app — apps/host's root
    // composition renders three `ChatApp`s over one session, and every `<LiveLab>`
    // on a documentation page mounts its own shell over one lab app. They are
    // views of one session, so they all show that session's question and
    // answering through any one of them settles it for all. Asserted rather than
    // assumed: this is the multiplicity #489 deliberately did NOT elect a leader
    // for, and it should fail a diff that quietly introduces one.
    const app = makeApp({
      namespace: 'tinytinkerer-docs-lab',
      presentation: 'modal',
    ...
    const answer = app.stores.humanPrompts.getState().request(pollView('One question'), 'conv-a')

    const first = await screen.findByTestId('surface-one')
    const second = screen.getByTestId('surface-two')
    expect(within(first).getByRole('dialog')).toHaveTextContent('One question')
    expect(within(second).getByRole('dialog')).toHaveTextContent('One question')

    fireEvent.click(within(second).getByRole('button', { name: 'Red' }))
