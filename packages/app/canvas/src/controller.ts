...
// The domain/controller binding stays schema-validated even though calls are now
// in-process. This preserves the former wire boundary's fail-fast input/result checks
// without retaining transport, nonce, or serialization infrastructure.
export const createExcalidrawHandlers = (
  ...
  ({
    ...
    search: defineCanvasMethod(excalidrawVerbContracts.search, (input) =>
      executeSearch(api, input)
    ),
    ...
    delete: defineCanvasMethod(excalidrawVerbContracts.delete, (input) =>
      executeDelete(api, input)
    ),
    ...
    survey: defineCanvasMethod(excalidrawVerbContracts.survey, (input) =>
      executeSurvey(api, input)
    ),
    preset: defineCanvasMethod(excalidrawVerbContracts.preset, (input) =>
      executePreset(api, input)
    ),
...
export const createCanvasController = (api: ExcalidrawImperativeAPI) => {
  const registrations = createExcalidrawHandlers(api)
