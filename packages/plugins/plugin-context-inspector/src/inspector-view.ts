...
// Pure mapper: a captured forwarded request → an InspectorView the host renders.
// Never touches React/DOM (enforced by scripts/check-boundaries.mjs). It does not
// return null — the host decides when to show the panel (enabled + at least one
// captured request); given a payload there is always something to show.
export const summarizeRequest: InspectorSummarizer = (
