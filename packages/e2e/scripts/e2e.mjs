...
// The suite now serves FOUR product shells, each on its own port (its own origin):
// web (/web/), widget (/widget/), mobile (/mobile/), and canvas (/canvas/). Allocate
// all four ports here, ONCE, before Playwright starts, so every Playwright process
// (runner + the config, which is re-evaluated per worker) agrees on the same base URLs.
//
// CI pins E2E_PORT (and may pin E2E_PORT_WIDGET / E2E_PORT_MOBILE / E2E_PORT_CANVAS).
// Locally we pick a random base port and derive the others as base+1 / base+2 / base+3,
// so parallel git worktrees don't collide. Each shell binds with --strictPort, so a
// collision fails fast rather than silently serving the wrong bundle.
