...
- Markdown parsing, Mermaid source detection, lazy loading, fallback behavior, and shared content wiring belong in `@tinytinkerer/content-markdown`, `@tinytinkerer/content-mermaid`, `@tinytinkerer/content-react`, and `@tinytinkerer/app-browser`. The Mermaid runtime is registered as a `NodeRendererPlugin` against the React content runtime from `@tinytinkerer/content-react`; specialized renderers should follow the same plugin shape.
