...
[`nostics`](https://github.com/vercel-labs/nostics) is a **developer-facing DX layer** for structured, code-named diagnostics (each carries a `why`, a `fix`, an optional `cause`, source locations, and a docs link) that replace ad-hoc `console.error('...')` / `throw new Error('...')` strings. It is **dev-only and strippable** — it does **not** change the production error model: the edge error contract (`edgeErrorResponseSchema`) and Sentry telemetry remain the prod source of truth.

- **One foundation module:** `packages/app/app-browser/src/diagnostics.ts` (`defineDiagnostics`). Codes are used at error sites via a relative import (`./diagnostics`), respecting the [Dependency Rules](#dependency-rules) — `nostics` is never pulled into the edge worker or a leaf package.
- **Stripped from production:** `@nostics/unplugin`'s strip transform (`nosticsStrip.vite()`, wired into every browser shell's vite config — `apps/web`, `apps/mobile`, `apps/widget`, since all three consume `app-browser` and deploy via `build:pages`) guards report-only diagnostic calls with `process.env.NODE_ENV !== 'production'` and pure-annotates the reporters, so they tree-shake out of the deployed bundle. Reporters are also gated on `import.meta.env.DEV`. A thrown `Diagnostic` (an `Error` subclass) is intentionally kept; its dev reporters are not.
- **Read in dev:** under `vite serve`, `nosticsCollector` (serve-only, in each shell's config) writes diagnostics to that shell's `.nostics.log`; read them with `node .agent/skills/nostics/tools/read-diagnostics.mjs`.
- See [diagnostics.md](./diagnostics.md) and the `.agent/skills/nostics/` skill for the full integration, when to use it vs `sentry-debugging`, and the time-boxed version waiver in `pnpm-workspace.yaml`.
