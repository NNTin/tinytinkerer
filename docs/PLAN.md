...
This checklist reflects the implemented migration state.
...
- [x] Split `apps/edge` into route modules and supporting transport modules
- [x] Keep edge stateless and contract-driven
- [x] Ensure route validation comes from shared contracts where appropriate
- [x] Remove monolithic route ownership from a single entry file
- [x] Keep edge isolated from browser and UI concerns

## Iteration 7: Add Widget Shell

- [x] Create `apps/widget` as an embeddable shell
- [x] Reuse `app-browser` as the widget's primary shared runtime dependency instead of copying web logic
- [x] Support host-token and OAuth-capable configurations through adapter config
- [x] Keep widget-specific embedding and layout concerns local to the app
- [x] Verify widget does not become a second copy of the web app internals

## Iteration 8: Shared Feature Packages

- [x] Define the rule for dedicated shared feature packages
- [x] Create feature packages only for large, reusable, non-trivial cross-app capabilities
- [x] Use Mermaid as the first explicit example of this rule
- [x] Treat direct app-to-feature imports as render-edge exceptions, not the default browser composition path
- [x] Keep feature packages downward-only and prevent them from bypassing `app-browser`
- [x] Keep `packages/ui` limited to primitives, not feature runtimes
- [x] Add boundary enforcement in CI for forbidden imports, package cycles, and browser-app bypasses around `app-browser`
