...
- **envelope and payload schema validation** — every inbound message's _envelope_ (`kind`, `verb`, ids, `protocolVersion`, `sessionNonce`) is re-parsed with Zod before it is acted on; a malformed or foreign-shaped message is dropped. Verb **payloads** use the same app-owned schema on both sides: the chat runtime validates before dispatch, and `createBridgeServer` validates a schema-bearing verb definition before invoking app code;
